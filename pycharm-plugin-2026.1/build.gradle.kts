...
intellijPlatform {
    pluginConfiguration {
        ...
        changeNotes = """
            <h3>0.4.1</h3>
            <ul>
              <li>Add PythonCore and Pythonid to plugin dependencies</li>
            </ul>
            <h3>0.4.0</h3>
            <ul>
    ...
    pluginVerification {
        ...
        ides {
            ...
            // = 261 / 261.*). JetBrains Plugin Verifier auto-discovers every
            // 2026.1.x release (2026.1, 2026.1.1, 2026.1.2, …) from the IDE
            ...
            // Forward-compat sanity: 2026.2 EAP (262.*). Outside this line's
            // declared `untilBuild`, so failures here do NOT block Marketplace
            // — they only warn us that bumping `untilBuild` to cover the next
            // major would currently break.
            select {
                types = listOf(IntelliJPlatformType.PyCharmProfessional)
