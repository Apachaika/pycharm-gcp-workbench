...
class RemoteGitPanel(
    ...
    private fun push() {
        ...
        if (result.exitCode == 0) {
            notifyPushSuccess(branch, result.output)
        } else {
            showResult(result)
        }
        historyLoaded = false
    ...
    private fun notifyPushSuccess(branch: String, output: String) {
        val hints = RemoteGitPushUrlExtractor.extract(output)
        val group = NotificationGroupManager.getInstance().getNotificationGroup("Vertex Workbench")
        val title = "Pushed '$branch'"
        val content = if (hints.isEmpty()) {
            "Vertex Workbench Git push succeeded."
        } else {
            buildString {
                append("Vertex Workbench Git push succeeded.")
                hints.forEach { append("\n").append(it.url) }
            }
        }
        val notification = group.createNotification(title, content, NotificationType.INFORMATION)
        hints.forEachIndexed { index, hint ->
            val label = if (hints.size == 1) hint.actionLabel else "${hint.actionLabel} #${index + 1}"
        ...
        if (hints.isNotEmpty()) {
            notification.addAction(object : NotificationAction("Copy URL") {
                override fun actionPerformed(e: AnActionEvent, n: Notification) {
                    CopyPasteManager.getInstance().setContents(StringSelection(hints.first().url))
                }
            })
        }
